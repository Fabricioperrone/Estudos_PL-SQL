   ...
   --resumindo ela procura  por dados idênticos nas tabelas envolvidas.

   --   Non-Equijoin
   -- A junção  non-Equijoin procura por relaciomentos que não correspondam a uma
   -- condição de igualdade, são geralmente utilizados  para verificar valores
   -- dentro de um certo conjunto de dados.

   --  Outer Joins
   -- Neste tipo de junção temos como resultado dados existentes em uma tabela  que não
   -- possuem uma condição  de igualdade com outra, ou seja, os registros de ligação são
   -- incluídos no resultado.
   -- Você pode utilizar um outer join para também visualizar as linhas que não correspondem a condição de Join
   -- O operador  de outer join é o sinal de adição (+).

   -- Self Joins
   -- Nas junções deste tipo são relacionados os dados de uma mesma tabela mais de uma vez, usando o mesmo
   -- raciocínio de uma Equijoin, só que nesse caso com a mesma tabela.
  ...
  SELECT TALUNO.COD_ALUNO, TALUNO.NOME, TCONTRATO.TOTAL
  FROM TALUNO, TCONTRATO
  WHERE TALUNO.COD_ALUNO = TCONTRATO.COD_ALUNO          -- critério de união de tabelas com where
  ...
  INSERT INTO TALUNO VALUES (10, 'PEDRO', 'NOVO HAMBURGO', NULL)  -- Não funcionou dessa forma
